...
function vzoom.execute_with_current_track_heights_locked(func, ...)
	...
	reaper.SetMediaTrackInfo_Value(master_track, "I_HEIGHTOVERRIDE", reaper.GetMediaTrackInfo_Value(master_track, "I_TCPH"))
...
-- Alias for get_max_track_h
function vzoom.get_tcp_height()
	return vzoom.get_current_max_track_height()
end
...
function vzoom.zoom_vertically_preserving_track_heights(
	...
	for i = 0, n_tracks - 1 do
		...
		-- Calculate new height
		new_h = heights[i] + heights[i] * scale_factor + height_increase

		-- Set new height enforcing min height
		heights[i] = math.max(new_h, min_h)
		::continue::
	end
